...
import { LeftArrowIcon, SongLetterIcon, ReplayIcon } from './icons';
import { BOT_SONG_LIST } from '../services/gameApi';

const TURN_DURATION = 20;

// Fix: Use 'as const' to infer a more specific type for BOTS.
// This ensures `bot.id` is typed as 'bot1' | 'bot2' | 'bot3' instead of string,
// which is compatible with the `ChatMessage['author']` type.
const BOTS = [
    { name: 'لحن', id: 'bot1' },
    { name: 'نغم', id: 'bot2' },
...
export const SongLetterSession: React.FC<{ onExit: () => void }> = ({ onExit }) => {
    ...
    const botTurn = (letter: string, currentMessages: ChatMessage[]) => {
        ...
        const possibleSongs = BOT_SONG_LIST.filter(song => song.title.startsWith(letter));
